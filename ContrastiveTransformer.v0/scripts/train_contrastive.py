...
def train_contrastive_model(rank, world_size, data, num_epochs=20, batch_size=32, learning_rate=0.001, use_distributed=False, checkpoint_path='checkpoint.pth', resume_checkpoint=True, port=9330):
    # set random status
    import random
    seed = 330
    random.seed(seed)
    ...
    if torch.cuda.is_available():
        ...
        model.to(device)
        projection_head = ProjectionHead(input_dim=hidden_dim, projection_dim=projection_dim).to(device)
        if use_distributed:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}", position=rank if use_distributed else 0) as pbar:
            ...
            for batch_idx, batch_data in enumerate(pbar):
                ...
                logging.info(f"Rank {rank}, Epoch {epoch+1}, Batch {batch_idx+1}: {batch_data[0].shape}")
